...
class DQNAgent:
    ...
    def get_actions(self, observations):
        """
        observations shape is (N,4) (batch size, observations size for cartpole)
        q_vals shape (N, 2) (batch size, no of actions for cartpole env)
...
def train_step(model, state_transitions, tgt, num_actions):
    ...
    with torch.no_grad():
        qvals_next = tgt(next_states).max(-1)[0] # shape of tgt(next_states): (N, num_actions)
